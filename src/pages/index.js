...
import { Trash ,BookCheck } from "lucide-react";
...
export default function Home() {
  ...
  const deleteCommit = () => {
    const updatedCards = commitLogs.filter(
      (itemCommit, index) => !discardingCommit.includes(itemCommit.commitId)
    );
    ...
    const revertedCommits = boards.map((insertItem, indexItem) => {
      selectedLog.forEach((e, i) => {
        ...
        if (e.toCard === insertItem.cardName) {
          const indexfound = insertItem.items.findIndex(
            (item) => item.title === e.title.title
          );
  ...
  const handleCommit = (element, item) => {
    const selectedIndex = commitLogs.findIndex(
      (ele) => ele.commitId === item.commitId
    );
    const selectedCommit = commitLogs.filter(
      (ele, ind) => ind >= selectedIndex
    );
    setSelectedLog(selectedCommit);
    const commitIdArray = commitLogs
      .map((commits, ind) => (ind >= selectedIndex ? commits.commitId : null))
  ...
  return (
    <div className="bg-cyan-300 min-h-screen">
      <div className="grid grid-cols-4 gap-4 pt-4 pl-7 ">
        ...
        >
          <div
            className="font-sans font-sans font-medium mb-1 mt-2 pb-4"
            ...
            {discardingCommit.length > 0 ? (
              ...
              >
                <motion.div
                  ...
                  className="rounded-lg text-xs flex cursor-pointer mr-10"
                >
                  <BookCheck color="#00a1ff" size={18} className="mr-2" />{" "}
                  <span className="pt-0.5">Keep</span>{" "}
                </motion.div>
                <motion.div
                ...
                >
                  <Trash color="#ef0b60" size={18} className="mr-2" />{" "}
                        ...
                        >
                          <LightTooltip
                            title={
                              <div>
                                ...
                                >
                                  <span
                                    className="text-sm font-sans font-semibold mr-2"
                                ...
                                <Typography
                                  className="font-sans font-sans text-sm"
                                  style={{ backgroundColor: "#ffffff" }}
                                >
                                  <span
                                    className="text-sm font-semibold  mr-2 font-sans"
                                    style={{ color: `${item.color}` }}
                                  >
                                    To Card:
                                  </span>{" "}
                                ...
                                >
                                  <span
                                    style={{ color: `${item.color}` }}
                                    className=" font-sans font-semibold mr-2 text-sm"
                                  >
                                    Title:
                                  </span>{" "}
                                  {item.title.title}
                          ...
                          >
                            <motion.div
                              ...
                              onHoverStart={(e) => {}}
                              style={
                                discardingCommit.includes(item.commitId)
                                  ? { backgroundColor: "#ef0b60" }
                                  : { backgroundColor: item.color }
                              }
                              onHoverEnd={(e) => {}}
                              onClick={() => handleCommit(index, item)}
                              key={index}
                    ...
                    {commitLogs
                      ...
                      ?.map((item, index) => (
                        <motion.div
                          ...
                          key={index}
                          className="text-xs ml-7 w-fit h-14 "
                        >
                          {item.time}
                        </motion.div>
                      ))}
